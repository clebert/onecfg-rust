...
#[derive(Debug)]
pub struct Onecfg {
  config_definition_by_path:
    indexmap::IndexMap<std::path::PathBuf, ConfigDefinition>,
  config_patches_by_path:
    indexmap::IndexMap<std::path::PathBuf, Vec<ConfigPatch>>,
}
...
impl Onecfg {
  ...
  pub fn generate_configs(
    &self,
  ) -> Result<indexmap::IndexMap<&std::path::Path, String>, Error> {
    let mut config_by_path = indexmap::IndexMap::new();

    for entry in &self.config_definition_by_path {
      let (path, config_definition) = entry;

      if let Some(mut config_patches) =
        self.config_patches_by_path.get(path).map(Clone::clone)
      {
        ...
        for config_patch in config_patches {
          crate::json::merge(
            &mut value,
            config_patch.value,
            &config_patch.array_merge,
          );
        }

        let config =
          config_definition.format.to_string(&value).ok_or_else(|| {
            Error::InvalidConfigPatchValue(path.display().to_string())
          })?;
  ...
  fn new() -> Self {
    Self {
      config_definition_by_path: indexmap::IndexMap::new(),
      config_patches_by_path: indexmap::IndexMap::new(),
    }
  }
  ...
  fn merge(&mut self, onecfg: Self) -> Result<(), Error> {
    for entry in onecfg.config_definition_by_path {
      ...
      let path = crate::path::normalize(&path).ok_or_else(|| {
        Error::IllegalConfigDefinitionPath(path.display().to_string())
      })?;

      if self.config_definition_by_path.contains_key(&path) {
        return Err(Error::DuplicateConfigDefinition(
    ...
    for entry in onecfg.config_patches_by_path {
      ...
      let path = crate::path::normalize(&path).ok_or_else(|| {
        Error::IllegalConfigPatchPath(path.display().to_string())
      })?;

      if let Some(existing_config_patches) =
        self.config_patches_by_path.get_mut(&path)
      {
...
impl PartialOnecfg {
  ...
  fn with_path(path: &std::path::Path) -> Result<Self, Error> {
    let file = std::fs::File::open(path).map_err(|source| {
      Error::FailedToReadOnecfgFile(path.display().to_string(), source)
    })?;

    let reader = std::io::BufReader::new(file);

    serde_json::from_reader(reader).map_err(|source| {
      Error::FailedToParseOnecfgFile(path.display().to_string(), source)
    })
  }
      ...
      Ok(
        serde_json::from_str(
          &reqwest::blocking::get(url)
            .map_err(|source| {
              Error::FailedToDownloadOnecfgFile(url.to_owned(), source)
            })?
            .text()
            .map_err(|source| {
              Error::FailedToDownloadOnecfgFile(url.to_owned(), source)
        ...
        .map_err(|source| {
          Error::FailedToParseOnecfgFile(url.to_owned(), source)
        })?,
      )
    } else if let Some(path) =
      url.split("file://").nth(1).map(std::path::Path::new)
    {
      Self::with_path(path)
    } else {
...
impl ConfigFormat {
  ...
  fn to_string(&self, value: &serde_json::Value) -> Option<String> {
    Some(match self {
      Self::Editorconfig => {
        toml::to_string_pretty(value).ok()?.replace('\"', "")
      },
...
fn test_inheritance() {
  let onecfg =
    load(std::path::Path::new("test/inheritance/onecfg.json")).unwrap();
